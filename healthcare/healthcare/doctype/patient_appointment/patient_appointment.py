...
from frappe.utils import flt, format_date, get_link_to_form, get_time, getdate, nowdate
...
@frappe.whitelist()
def create_sales_invoice_from_appointment(
    appointment_name,
    patient,
    appointment_type,
    cost_center,
    ...
    try:
        ...
        if appointment.sales_invoice:
            if frappe.db.exists('Sales Invoice', appointment.sales_invoice):
                frappe.throw(
                    _("Sales Invoice <b>{0}</b> already created for this appointment.")
                    .format(appointment.sales_invoice)
                )
            else:
                appointment.db_set('sales_invoice', '')

        appointment_type_doc = frappe.get_doc('Appointment Type', appointment_type)

        item_code = None
        op_consulting_charge = None

        for item in appointment_type_doc.items:
            if item.op_consulting_charge_item:
                item_code = item.op_consulting_charge_item
                op_consulting_charge = flt(item.op_consulting_charge)
                break

        if not item_code:
            frappe.throw(_("OP Consulting Charge Item not found"))

        if not op_consulting_charge:
            frappe.throw(_("OP Consulting Charge Rate not found"))

        customer = frappe.db.get_value('Patient', patient, 'customer')
        if not customer:
            frappe.throw(_("No Customer linked to Patient"))

        sales_invoice = frappe.new_doc('Sales Invoice')
        sales_invoice.patient = patient
        sales_invoice.customer = customer
        sales_invoice.cost_center = cost_center
        sales_invoice.type = "Clinic"
        sales_invoice.discount_amount = discount_amount
        sales_invoice.additional_discount_percentage = discount_percentage
        ...
        sales_invoice.append('items', {
            ...
            'description': f"Consultation Charge for {appointment_type}"
        })

        sales_invoice.save()
        sales_invoice.submit()

        appointment.db_set('sales_invoice', sales_invoice.name)
...
def get_default_receivable_account():
    ...
    if company:
        return frappe.get_value('Company', company, 'default_receivable_account')
    return None

def get_item_rate(item_code):
    """
    Get item rate from Price List or Item master
    """
    # Get standard selling rate from Item
    item = frappe.get_doc('Item', item_code)
    if item.standard_rate:
        return item.standard_rate
    
    # Alternatively, fetch from Price List
